...
logging.basicConfig(
    filename='logs.txt', format='%(asctime)s - %(name)s - %(levelname)s - %(message)s', level=logging.INFO
)
...
def get_repl_logs(update: Update, context):
    ...
    try:
        with open("/var/log/postgresql/postgresql-15-main.log", "r") as f:
            text = f.readlines()
        result = "```\n"
        for i in range(len(text)):
            if "received replication command" in text[i]:
                if len(result + text[i] + text[i + 1]) <= 4096:
                    result += text[i]
                    result += text[i + 1]
                else:
                    result += "```"
